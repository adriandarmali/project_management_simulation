...
[data-testid="stSidebarCollapseButton"] svg { color: white !important; }
/* Expand arrow when collapsed — make it visible */
...
def render_sidebar():
    with st.sidebar:
        ...
        if st.button("Close Form" if add_open else "+ Add Activity", type="secondary" if add_open else "primary",
...
def main():
    ...
    with tb1:
        if st.button("📂 Import / Export CSV", use_container_width=True):
            st.session_state.csv_open = not st.session_state.get("csv_open", False)
            st.rerun()
    with tb2:
    ...
    if st.session_state.get("csv_open", False):
        with st.container():
            st.markdown('<div style="background:#fafafa;border:1px solid #e2e8f0;border-radius:8px;padding:18px 20px;margin:10px 0">', unsafe_allow_html=True)
            cc1, cc2 = st.columns(2)
            with cc1:
                st.markdown("**Download Template**")
                st.download_button("⬇ CSV Template", data=CSV_TEMPLATE,
                                   file_name="project_template.csv", mime="text/csv",
                                   use_container_width=True)
            with cc2:
                st.markdown("**Upload CSV**")
                uploaded = st.file_uploader("", type=["csv"], key="csv_up", label_visibility="collapsed")
                if uploaded:
                    raw = uploaded.read().decode("utf-8")
                    existing = [a["label"] for a in st.session_state.activities]
                    rows, ferrs = parse_csv(raw, existing)
                    if ferrs:
                        for e in ferrs: st.error(e)
                    else:
                        has_errs = any(r["errors"] for r in rows)
                        prev = [{"Label": r["activity"]["label"], "Name": r["activity"]["name"],
                        ...
                        if not has_errs:
                            ...
                            if st.button(f"Import {len(clean)} activities", type="primary", use_container_width=True):
                                st.session_state.activities = clean if mode == "Replace" else st.session_state.activities + clean
                                st.session_state.sim_results = None
                                st.session_state.csv_open = False; st.rerun()
                        else:
                            st.error("Fix errors above before importing.")
            st.markdown('</div>', unsafe_allow_html=True)
